...
in
{
  config = lib.mkIf cfg.enable {
    ...
    ## Sec for VM - block claude VM from reaching host while allowing SSH to VM
    networking.firewall.extraCommands = ''
      iptables -I INPUT 1 -i vboxnet0 -m state --state ESTABLISHED,RELATED -j ACCEPT
      iptables -I INPUT 2 -i vboxnet0 -j DROP
      iptables -I FORWARD 1 -i vboxnet0 -m state --state ESTABLISHED,RELATED -j ACCEPT
      iptables -I FORWARD 2 -i vboxnet0 -j DROP
    '';

    networking.firewall.extraStopCommands = ''
      iptables -D INPUT -i vboxnet0 -m state --state ESTABLISHED,RELATED -j ACCEPT 2>/dev/null || true
      iptables -D INPUT -i vboxnet0 -j DROP 2>/dev/null || true
      iptables -D FORWARD -i vboxnet0 -m state --state ESTABLISHED,RELATED -j ACCEPT 2>/dev/null || true
